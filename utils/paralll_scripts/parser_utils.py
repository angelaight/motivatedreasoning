...
class caseMetadataExtractor():
    ...
    def get_citations(self):
        if self.courtCaseDocBody is not None:
                ...
                for classification_item in classification_items:
                    try:
                        ...
                        for sub_classification_item in classification_item.find_all('classificationitem'):
                            ...
                            classes.append(
                                (classificationscheme, classificationscheme_finer, score, class_name, class_code))
...
def standardize_docket_num(s):
    ...
    if len(nums) == 2:
        ...
        for i in range(len(nums) // 2):
            padded = padding_function(*nums[2 * i:2 * i + 2])
            if padded:
                ret_list.append(padded)
            if len(nums) % 2 == 1:
        ...
        for item in c[3::2]:
            ...
            for doc in case_meta['citingCases']:
                ...
                for item in doc_details_set:
                    doc_details.append((item[0], dict(
                        zip(('cite_type', 'normalized_cite', 'locator_keyname', 'locator_keyvalue'), item[1:]))))
...
def parse_zip(zip_path, outfile):
    ...
    for path in xml_list:
        ...
        try:
            ...
            print("AttributeError when parsing {}".format(zip_path))
